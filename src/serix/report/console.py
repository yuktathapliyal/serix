...
def print_attack_results(results: AttackResults) -> None:
    ...
    panel_content += f"[bold]Attempts:[/bold] {results.total_attempts}\n"
    ...
    for i, attack in enumerate(results.attacks):
        result_str = "[red]FAIL[/red]" if attack.success else "[green]PASS[/green]"
        payload_preview = (
    ...
    if results.successful_attacks:
        console.print(
            f"\n[bold red]⚠️  VULNERABLE:[/bold red] Agent was compromised by "
